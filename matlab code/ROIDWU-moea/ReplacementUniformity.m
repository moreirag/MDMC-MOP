function [Remain] = ReplacementUniformity(Population,N,Front,Weight,penalty,angle)
...
% uniformity measure.
    ...
    if sum(ind) == 1
        Aux.decs = B.decs(ind,:);
        Aux.wgts = B.wgts(ind,:);
        Aux.penalty = B.penalty(ind,:);
        Aux.angle = B.angle(ind,:);
        Aux.next = Next(ind);
        %
        nFront = Front(~ind);
        C.decs = B.decs(~ind,:);
        C.wgts = B.wgts(~ind,:); 
        C.penalty = B.penalty(~ind,:);
        C.angle = B.angle(~ind,:);
        nNext = Next(~ind);
        ind1 = nFront == min(nFront);
        Aux.decs = [Aux.decs;C.decs(ind1,:)];
        Aux.wgts = [Aux.wgts;C.wgts(ind1,:)];
        Aux.penalty = [Aux.penalty;C.penalty(ind1,:)];
        Aux.angle = [Aux.angle;C.angle(ind1,:)];
        Aux.next = [Aux.next nNext(ind1)];
        [D,I] = distDWU(Aux,Aux,-1);
        [~, j] = max(D);
        R.decs(ii,:) = Aux.decs(j,:);
        R.wgts(ii,:) = Aux.wgts(j,:);
        R.penalty(ii,:) = Aux.penalty(j,:);
        R.angle(ii,:) = Aux.angle(j,:);
        ii = ii + 1;
        R.decs(ii,:) = Aux.decs(I(j),:);
        R.wgts(ii,:) = Aux.wgts(I(j),:);
        R.penalty(ii,:) = Aux.penalty(I(j),:);
        R.angle(ii,:) = Aux.angle(I(j),:);
        B.decs(Aux.next([j I(j)]),:)=[];
        B.wgts(Aux.next([j I(j)]),:)=[];
        B.penalty(Aux.next([j I(j)]),:)=[];
        B.angle(Aux.next([j I(j)]),:)=[];
    ...
    while ii < N
        ...
        R.decs(ii,:) = B.decs(j,:);
        R.wgts(ii,:) = B.wgts(j,:);
        R.penalty(ii,:) = B.penalty(j,:);
        R.angle(ii,:) = B.angle(j,:);
        B.decs(j,:) = [];
        B.wgts(j,:) = [];
        B.penalty(j,:) = [];
        B.angle(j,:) = [];
...
function [D,I] = distDWU(X,Y,varargin)
    ...
    for i = 1:ny
        ...
        dsq = sqrt(dsq);
        %dsq = dsq./((wgts+1));
        dsq = dsq./(wgts+1)-1*(WP(i)*(1*exp((WA(i) - theta))));
        %dsq = dsq./(wgts+1+1*WP(i)*(exp((WA(i) - theta))));
